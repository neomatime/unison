    ...
    <aside role="dialog" aria-modal="true" aria-label={title} className="unison-drawer-enter absolute top-0 right-0 flex h-full w-full max-w-md flex-col border-l border-border bg-card shadow-2xl">
      <header className="flex items-center justify-between border-b border-border p-5">
        <div className="flex items-center gap-3">
          ...
          <h2 className="unison-section-title text-sm">{title}</h2>
        </div>
        <button type="button" onClick={onClose} aria-label="Close panel" className="p-2 transition-colors hover:bg-muted"><X className="size-4" /></button>
      </header>
...
function HelpContent() {
  ...
  return <div className="space-y-3">
    {resources.map(([Icon, title, description]) => <button type="button" key={title} onClick={() => setSelected(title)} className="flex w-full items-center gap-4 rounded-none border border-border p-4 text-left transition-colors hover:bg-muted/40">
      <span className="flex size-10 items-center justify-center border border-border bg-muted/40"><Icon className="size-5" /></span>
      <span><span className="unison-record-name block text-sm">{title}</span><span className="text-xs text-muted-foreground">{description}</span></span>
    </button>)}
    <div className="mt-6 border border-border bg-muted/35 p-4">
      <p className="text-sm font-medium">{selected || 'UNISON workspace help'}</p>
...
function NotificationContent() {
  ...
  return <div>
    <div className="flex items-center justify-between"><p className="unison-metric-label text-xs text-muted-foreground">Today</p><button type="button" onClick={() => setRead(notifications.map(() => true))} className="text-xs font-medium text-brand">Mark all read</button></div>
    <div className="mt-3 space-y-2">{notifications.map(([title, meta], index) => <button type="button" key={title} onClick={() => setRead((current) => current.map((value, itemIndex) => itemIndex === index ? true : value))} className="flex w-full gap-3 rounded-none border border-border p-4 text-left transition-colors hover:bg-muted/40">
...
function SearchContent({ onClose }: { onClose: () => void }) {
  ...
  return <div>
    <label className="relative block"><Search className="absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search records, people and knowledge..." className="h-10 w-full rounded-none border border-border bg-background pr-3 pl-9 text-sm outline-none transition-colors focus:border-ring focus:ring-2 focus:ring-ring/15" /></label>
    <p className="unison-metric-label mt-6 text-xs text-muted-foreground">{query ? 'Search results' : 'Suggested results'}</p>
    <div className="mt-2 space-y-1">{results.map(([category, label, href]) => <Link key={href} href={href} onClick={onClose} className="flex items-center justify-between px-3 py-3 transition-colors hover:bg-muted"><span><span className="block text-xs text-muted-foreground">{category}</span><span className="unison-record-name block text-sm">{label}</span></span><Command className="size-4 text-muted-foreground" /></Link>)}{results.length === 0 ? <p className="border border-border bg-muted/35 p-5 text-center text-sm text-muted-foreground">No records match “{query}”.</p> : null}</div>
  </div>
}
