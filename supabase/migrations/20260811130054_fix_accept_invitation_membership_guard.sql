...
begin
  ...
  if existing_membership.id is not null then
    if existing_membership.status = 'active' then
      -- The caller already holds an active membership. We deliberately do
      -- not touch it (never let an invitation mutate an existing active
      -- row). We also cannot mark this invitation 'accepted' here even
      -- though it is spent: the raise below aborts the whole transaction
      -- and would roll back that update too, for the identical reason
      -- given in the expiry branch above. Leaving it 'pending' is a known
      -- gap, tracked under the same follow-up as the expiry cleanup.
      raise exception 'caller is already a member of this organization' using errcode = '23505';
    elsif existing_membership.status in ('suspended', 'removed') then
