...
Clients create/edit/archive controls call real server actions against the database (`features/clients/actions`), scoped to the active organization and subject to RLS. Sign-in, invitation send/accept, and tenant switching are also real — they hit Supabase Auth and the `memberships`/`invitations`/`accept_invitation` machinery described in `docs/tenancy.md`, not local state. Everywhere else, controls remain demonstrative: connect/save buttons on the sixteen unconnected modules do not call external services, and module toggles, Atlas prompts, approvals, and automation are local UI only. Password-reset and verification mail still come from Supabase's own sender: Auth's mailer only speaks SMTP, which Microsoft 365 blocks on this tenant, so it cannot use the Graph route invitation mail now takes. No invitation email has ever actually been sent either — the send/accept flow is verified against a log transport, not a live mailbox.
