...
- `pnpm test:rls` runs `tests/integration/rls/*.test.ts` against the **live** Supabase project. It requires `.env.local` (`--env-file=.env.local`) populated from `.env.example`, including `SUPABASE_SECRET_KEY`. These specs create and tear down real rows, so treat them as integration tests against a real backend, not a mock.
