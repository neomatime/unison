...
function EventDrawer({ event, onClose }: { event: (typeof calendarEvents)[number]; onClose: () => void }) {
  ...
  const editHref = `/operations/calendar/events/edit?title=${encodeURIComponent(event.title)}&day=${event.day}&time=${encodeURIComponent(event.time)}&owner=${encodeURIComponent(event.owner)}`
  return <div className="fixed inset-0 z-50"><button type="button" aria-label="Close event" onClick={onClose} className="absolute inset-0 bg-foreground/20" /><aside className="absolute top-0 right-0 h-full w-full max-w-md border-l border-border bg-card p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-semibold text-brand">CALENDAR EVENT</p><h2 className="mt-2 text-xl font-bold">{event.title}</h2></div><button type="button" aria-label="Close" onClick={onClose} className="rounded-lg p-2 hover:bg-muted"><X className="size-4" /></button></div><div className="mt-8 space-y-4 rounded-xl bg-muted/50 p-5 text-sm"><p><span className="text-muted-foreground">When</span><span className="float-right font-medium">{event.day} Aug · {event.time}</span></p><p><span className="text-muted-foreground">Owner</span><span className="float-right font-medium">{event.owner}</span></p><p><span className="text-muted-foreground">Location</span><span className="float-right font-medium">HIMARK Workspace</span></p></div><div className="mt-6 flex gap-2"><Link href={editHref} className="rounded-lg bg-foreground px-3 py-2 text-sm font-semibold text-primary-foreground">Edit event</Link><button type="button" onClick={() => setMessage('Event summary was shared.')} className="rounded-lg border border-border px-3 py-2 text-sm font-semibold">Share</button></div>{message ? <p role="status" className="mt-4 text-sm font-medium text-brand">{message}</p> : null}</aside></div>
}
...
function InlineNotice({ message, onClose }: { message: string; onClose: () => void }) {
  return <div role="status" className="mt-4 flex items-center justify-between border border-brand/20 bg-brand-soft px-4 py-3 text-sm font-medium text-brand"><span>{message.replace(/ drawer opened/i, ' opened')}</span><button type="button" aria-label="Dismiss message" onClick={onClose}><X className="size-4" /></button></div>
}
