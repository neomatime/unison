...
export function ModuleForm({ module, mode, recordId }: { module: ModuleDefinition; mode: 'create' | 'edit'; recordId?: string }) {
  ...
  const fields = domainFields[module.id] ?? module.fields
  ...
  return <>
    ...
    <form onSubmit={submit} className="mx-auto max-w-5xl space-y-5">
      {sections.map((section) => <section key={section} className="rounded-xl border border-border bg-card p-6 shadow-[0_1px_2px_rgb(16_32_46_/_0.04)]"><div className="mb-5"><h2 className="font-semibold text-foreground">{section}</h2><p className="mt-1 text-sm text-muted-foreground">Provide the information used across the {module.label.toLowerCase()} workspace.</p></div><div className="grid gap-5 md:grid-cols-2">{fields.filter((field) => field.section === section).map((field) => <Field key={field.name} field={field} value={record?.[field.name] ?? (field.name === 'name' ? record?.name : '')} />)}</div></section>)}
      <div className="sticky bottom-4 flex items-center justify-between rounded-xl border border-border bg-card/95 px-5 py-4 shadow-xl backdrop-blur"><p className="flex items-center gap-2 text-xs text-muted-foreground"><Info className="size-4" />Required fields and ownership are validated before saving.</p><div className="flex gap-2"><Link href={record ? `${module.route}/${record.id}` : module.route} className="rounded-lg border border-border px-4 py-2 text-sm font-medium">Cancel</Link><button type="button" onClick={() => setSaved(true)} className="rounded-lg border border-border px-4 py-2 text-sm font-medium">Save Draft</button><button type="submit" className="rounded-lg bg-foreground px-4 py-2 text-sm font-semibold text-primary-foreground">{mode === 'create' ? `Create ${module.singular}` : 'Save Changes'}</button></div></div>
    </form>
