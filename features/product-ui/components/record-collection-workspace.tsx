...
export function RecordCollectionWorkspace({ config, compact = false, onPrimaryAction }: { config: CollectionConfig; compact?: boolean; onPrimaryAction?: () => void }) {
  ...
  return <>
    <section className="overflow-hidden rounded-xl border border-border bg-card">
      ...
      {visible.length ? <div className="overflow-x-auto"><table className={`w-full text-left ${compact ? 'min-w-[720px]' : 'min-w-[920px]'}`}><thead><tr className="bg-muted/30 text-[0.65rem] font-semibold tracking-wide text-muted-foreground uppercase"><th className="w-12 px-5 py-3"><input type="checkbox" aria-label="Select all visible records" checked={paged.length > 0 && paged.every((record) => selected.includes(record.id))} onChange={(event) => setSelected(event.target.checked ? [...new Set([...selected, ...paged.map((record) => record.id)])] : selected.filter((id) => !paged.some((record) => record.id === id)))} /></th>{columns.map((column) => <th key={column.id} className="px-4 py-3">{column.label}</th>)}<th className="w-16 px-4 py-3">Actions</th></tr></thead><tbody>{paged.map((record) => <tr key={record.id} className="group border-t border-border hover:bg-muted/25"><td className="px-5 py-3.5"><input type="checkbox" aria-label={`Select ${record.name}`} checked={selected.includes(record.id)} onChange={(event) => setSelected(event.target.checked ? [...selected, record.id] : selected.filter((id) => id !== record.id))} /></td>{columns.map((column, index) => <td key={column.id} className="max-w-64 px-4 py-3.5 text-xs">{index === 0 ? <a href={recordRoute(record)} onClick={hasConfiguredRecordRoute ? undefined : (event) => { event.preventDefault(); openRecord('view', record) }} className="unison-record-name unison-action-control text-left text-sm hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/20">{String(record[column.id] ?? record.name)}</a> : column.id === 'status' ? <HealthBadge>{String(record.status)}</HealthBadge> : <span className="text-muted-foreground">{String(record[column.id] ?? '—')}</span>}</td>)}<td className="px-4 py-3.5"><RowActionMenu label={record.name} actions={record.archived ? [
    ...
    {message ? <button type="button" role="status" aria-live="polite" onClick={() => setMessage('')} className="unison-live-region unison-action-control fixed right-6 bottom-6 z-[100] max-w-sm rounded-xl bg-foreground px-4 py-3 text-left text-sm font-medium text-primary-foreground shadow-xl">{message}<X className="ml-3 inline size-3.5 opacity-60" /></button> : null}
  </>
}
