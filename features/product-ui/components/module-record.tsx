      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="relative flex flex-wrap gap-2">
          ...
          {moreOpen ? <div className="absolute top-full right-0 z-30 mt-2 w-48 rounded-xl border border-border bg-card p-2 text-sm shadow-xl">
            ...
            {moduleActions(module.id).map((action) => <Link key={action} href={`/records/action?module=${module.id}&record=${record.id}&action=${encodeURIComponent(action)}`} onClick={() => setMoreOpen(false)} className="block w-full rounded-lg px-3 py-2 text-left hover:bg-muted">{action}</Link>)}
...
function RecordTabContent({ activeTab, module, record, onAction }: { activeTab: string; module: ModuleDefinition; record: FixtureRecord; onAction: (message: string) => void }) {
  ...
  if (['Documents', 'Receipt', 'Equipment', 'Versions'].includes(activeTab)) return <FileCollection title={activeTab} returnHref={`${module.route}/${record.id}`} onAction={onAction} />
  if (['Contacts', 'Team', 'Candidates', 'Attendees'].includes(activeTab)) return <PeopleCollection title={activeTab} module={module} record={record} />
  if (['Tasks', 'Subtasks', 'Checklist', 'Dependencies', 'Milestones', 'Deliverables'].includes(activeTab)) return <WorkCollection title={activeTab} module={module} record={record} />
...
function FileCollection({ title, returnHref, onAction }: { title: string; returnHref: string; onAction: (message: string) => void }) {
  const files = ['Executive summary.pdf', 'Scope and requirements.docx', 'Commercial schedule.xlsx']
  return <><div className="flex items-center justify-between"><TabIntro tab={title} singular="record" /><Link href={`/records/documents/upload?return=${encodeURIComponent(returnHref)}`} className="rounded-lg bg-foreground px-3 py-2 text-sm font-semibold text-primary-foreground">Upload file</Link></div><div className="mt-6 divide-y divide-border rounded-xl border border-border">{files.map((file, index) => <button type="button" key={file} onClick={() => onAction(`${file} is ready to preview.`)} className="flex w-full items-center gap-3 p-4 text-left hover:bg-muted/40"><Paperclip className="size-4 text-muted-foreground" /><span><span className="block text-sm font-medium">{file}</span><span className="text-xs text-muted-foreground">{index + 1}.{index + 2} MB · Updated this week</span></span><span className="ml-auto text-xs text-muted-foreground">Preview</span></button>)}</div></>
}

function recordActionHref(module: ModuleDefinition, record: FixtureRecord, action: string, subject?: string) {
  const query = new URLSearchParams({ module: module.id, record: record.id, action })
  if (subject) query.set('subject', subject)
  return `/records/action?${query.toString()}`
}

function PeopleCollection({ title, module, record }: { title: string; module: ModuleDefinition; record: FixtureRecord }) {
  ...
  return <>
    <div className="flex items-center justify-between"><TabIntro tab={title} singular="record" /><Link href={recordActionHref(module, record, `Add ${title}`)} className="rounded-lg border border-border px-3 py-2 text-sm font-semibold">Add</Link></div>
    <div className="mt-6 grid gap-3 sm:grid-cols-2">{people.map((person, index) => <article key={person.name} className="flex items-center gap-3 rounded-xl border border-border p-4"><span className="flex size-9 items-center justify-center rounded-full bg-foreground text-xs font-bold text-primary-foreground">{person.name.split(' ').map((part) => part[0]).join('')}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{person.name}</span><span className="text-xs text-muted-foreground">{person.role}</span></span><Link aria-label={`Edit ${person.name}`} href={recordActionHref(module, record, `Edit ${title}`, person.name)} className="rounded-md p-2 text-muted-foreground hover:bg-muted"><Pencil className="size-3.5" /></Link><button type="button" aria-label={`Remove ${person.name}`} onClick={() => setPeople((current) => current.filter((_, personIndex) => personIndex !== index))} className="rounded-md p-2 text-muted-foreground hover:bg-muted hover:text-destructive"><Archive className="size-3.5" /></button></article>)}</div>
  </>
}

function WorkCollection({ title, module, record }: { title: string; module: ModuleDefinition; record: FixtureRecord }) {
  ...
  return <><div className="flex items-center justify-between"><TabIntro tab={title} singular="record" /><Link href={recordActionHref(module, record, `Add ${title}`)} className="rounded-lg border border-border px-3 py-2 text-sm font-semibold">Add {title.toLowerCase()}</Link></div><div className="mt-6 divide-y divide-border rounded-xl border border-border">{items.map((item, index) => <div key={`${item.name}-${index}`} className="flex items-center gap-3 p-4"><input aria-label={`Complete ${item.name}`} type="checkbox" checked={item.done} onChange={() => setItems((current) => current.map((entry, itemIndex) => itemIndex === index ? { ...entry, done: !entry.done } : entry))} className="size-4" /><span className={item.done ? 'flex-1 text-sm text-muted-foreground line-through' : 'flex-1 text-sm font-medium'}>{item.name}</span><Link aria-label={`Edit ${item.name}`} href={recordActionHref(module, record, `Edit ${title}`, item.name)} className="rounded-md p-2 text-muted-foreground hover:bg-muted"><Pencil className="size-3.5" /></Link><button type="button" aria-label={`Archive ${item.name}`} onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="rounded-md p-2 text-muted-foreground hover:bg-muted hover:text-destructive"><Archive className="size-3.5" /></button></div>)}</div></>
}
