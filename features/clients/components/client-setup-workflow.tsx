...
function StakeholderDrawer({ open, onClose, onAdd }: { open: boolean; onClose: () => void; onAdd: (stakeholder: Stakeholder) => void }) {
  ...
  if (!open) return null
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-5"><button type="button" aria-label="Close stakeholder form" onClick={onClose} className="absolute inset-0 bg-foreground/25" /><section role="dialog" aria-modal="true" aria-labelledby="stakeholder-form-title" className="relative w-full max-w-3xl border border-border bg-card shadow-2xl"><header className="flex items-start justify-between border-b border-border p-6"><div><h2 id="stakeholder-form-title" className="text-lg font-bold">Add Stakeholder</h2><p className="mt-1 text-sm text-muted-foreground">Add another client contact within the current full-page client setup workflow.</p></div><button type="button" onClick={onClose} className="border border-border p-2"><X className="size-4" /></button></header><form onSubmit={(event) => { event.preventDefault(); onAdd({ name, role, type, email }) }}><div className="grid gap-5 p-6 sm:grid-cols-2"><Field label="Full Name" value={name} onChange={setName} /><Field label="Job Title" value={role} onChange={setRole} /><Field label="Stakeholder Type" value={type} onChange={setType} options={['Decision Maker', 'Technical Stakeholder', 'Billing Stakeholder', 'Project Stakeholder']} /><Field label="Email" type="email" value={email} onChange={setEmail} /></div><footer className="flex justify-end gap-2 border-t border-border p-5"><button type="button" onClick={onClose} className="border border-border px-4 py-2 text-sm font-semibold">Cancel</button><button type="submit" className="bg-foreground px-4 py-2 text-sm font-semibold text-primary-foreground">Add Stakeholder</button></footer></form></section></div>
}
